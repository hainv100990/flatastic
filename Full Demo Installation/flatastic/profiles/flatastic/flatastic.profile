 ...
 */
function flatastic_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'flatastic_form_submit';
}


function flatastic_form_submit($form, &$form_state) {
  // Import database flatastic file.
  $flatastic_file = dirname(__FILE__) . '/flatastic.sql';
  $success = import_flatastic($flatastic_file);
...
/**
 * Imports a database flatastic file.
 *
 * @see demo_reset().
 */
function import_flatastic($filename) {
  ...
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open flatastic file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
